...
## Differences between Dextinity Admin `FinalForm` and React Final Form's `Form`
...
Dextinity Admin `FinalForm` renders some HTML: a `<form>` wrapper and a div containing submit errors.
Additionally, it uses the React Contexts of other Dextinity Admin components to interact with them.
You can find more information about these interactions in the following sections.

The following example shows the definition of a simple form with RFF `Form` and Dextinity Admin `FinalForm`.
...
Dextinity Admin `FinalForm`:

<Source
    language="tsx"
    code={dedent`
        import { FinalForm, Field, SaveButton } from "@dextinity/admin";
        // ...
        <FinalForm
            mode="add"
            onSubmit={(values) => {
