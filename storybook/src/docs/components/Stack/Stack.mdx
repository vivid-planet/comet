...
Dextinity Admin Stack is a component for creating an application with stacked pages allowing the user to navigate between them.
...
Dextinity Admin Stack uses react-router behind the scene and creates a `Route` for every page.
...
Dextinity Admin Stack provides a unique way to create a StackSwitch associated with its api using `useStackSwitch` hook:
...
Toolbar components that integrate stack nicely into the Dextinity Toolbar System.
