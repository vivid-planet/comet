...
In Dextinity Admin, a _selection_ can be used to manage which item of a list or table a user has currently selected.

The two most common use cases for selections are

- creating and editing items in a dialog.
  Dextinity Admin has a dedicated `EditDialog` component that uses selection internally.
...
Dextinity Admin has this functionality built-in.
