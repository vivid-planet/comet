...
The Dextinity Admin `Table` internally uses [Material UI's `Table` component](https://v4.mui.com/components/tables/) and its [associated components](https://v4.mui.com/components/tables/#api).
...
In the following text, `Table` refers to the Dextinity Admin Table, while `MuiTable` refers to Material UI's Table.

## Differences between Dextinity Admin `Table` and `MuiTable`
...
Dextinity Admin `Table` adds a layer of abstraction to `MuiTable`.
...
|                                                                                          | Material UI `Table` | Dextinity Admin `Table` |
| ---------------------------------------------------------------------------------------- | ------------------- | ----------------------- |
...
The following example shows the definition of a simple table with `MuiTable` and Dextinity Admin `Table`.
...
In Dextinity Admin `Table`, it's sufficient to provide an object containing column definitions:
...
However, Dextinity Admin `Table` is not limited to simple tables.
