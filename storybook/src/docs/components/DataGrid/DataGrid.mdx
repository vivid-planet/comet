...
Dextinity Admin contains various helpers for using MUI X DataGrid. It works with the MIT or the commercial version.
...
```ts
import { GridColDef, dataGridDateColumn, dataGridDateTimeColumn } from "@dextinity/admin";
...
When defining the columns, use the `GridColDef` type from `@dextinity/admin` instead of `@mui/x-data-grid`.
...
## DataGrid to Dextinity GraphQL API Converter

Dextinity Admin comes with three helpers that convert MUI DataGrid state to standard Dextinity GraphQL API variables.
