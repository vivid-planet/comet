...
Used to show and hide individual columns depending on the screen size.

For example, the columns "First name" and "Last name" are shown by default, but if the screen size is below 900px, the column "Full name" is shown instead.

<Canvas>
    <Story id="stories-components-datagrid--usedynamicgridvisibilitymodel" />
</Canvas>
