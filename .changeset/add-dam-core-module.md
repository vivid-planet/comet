...
Add `DamCoreModule` for using DAM file upload and download without the full DAM
...
The file upload/download stack has been extracted into a new lightweight `DamCoreModule`. It provides the `FilesService`, `FoldersService` and the file/folder HTTP endpoints (upload, replace, stream, download, zip) and only requires `BlobStorageModule`:
...
`DamModule` now builds on top of `DamCoreModule` and adds the GraphQL resolvers, image serving, blocks and the user-permission based access control. Existing usage of `DamModule` is unchanged.
