...
A new `NotFound` component is rendered in `MasterMenuRoutes` for unmatched top-level routes. The same fallback now also applies to nested navigation: `StackSwitch`, `RouterTabs` and `BlockAdminTabs` render the configured `NotFound` instead of silently falling back to the initial page or default tab when the URL points to an unknown sub-route.
