...
`<ActionLogButton />` is the recommended entry point. It renders a button that opens an `<ActionLogDialog />` modal containing the version history.

#### ActionLogButton

Place the component in your entity's edit-page toolbar. It renders the button and manages the dialog's open/close state for you:
...
export function EditProduct({ id, name }: { id: string; name?: string }) {
    return (
        <>
            <Toolbar>
                <ToolbarBackButton />
                <ToolbarTitleItem>{name}</ToolbarTitleItem>
                <ToolbarFillSpace />
                <ToolbarActions>
...
If you need to open the dialog from elsewhere (a menu item, a row action, …), use `<ActionLogDialog />` directly and manage the `open` state yourself.

**Example** – click the button in the toolbar to open the dialog:

<StorybookCmsAdminStoryIframe storyId="action-log-action-log-button--in-edit-page" height={900} />

The dialog itself has three views:
...
For the full list of props, see the [Action log stories in Storybook](https://storybook.comet-dxp.com/?path=/story/comet-cms-admin-action-log-action-log-button--default).
