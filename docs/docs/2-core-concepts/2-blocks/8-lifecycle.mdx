...
- The `BlockInterface#input2State` method is used to transition from input to state.
    ...
    The block's state may be typed differently than the block's input.
    Use the `BlockState` helper type to transform the type:

    ```ts
    type HeadlineBlockState = BlockState<typeof HeadlineBlock>;
    ```
