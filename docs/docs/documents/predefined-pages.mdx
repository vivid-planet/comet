...
**Example:** The news page in the code at `https://example.com/news` should be rendered in the page tree at `https://example.com/company/news`.
We need the following redirect and rewrite:

-   A **rewrite** from `/company/news` to `/news` (to ensure that the page can be viewed at the expected location)
-   A **redirect** from `/news` to `/company/news` (to avoid duplicate content)
...
If you want to add a new type (e.g., products), the following changes must be made:
