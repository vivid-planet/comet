...
<!-- TODO: ein vergleich mit einem DTO und/oder GraphQL Input Type kann vielleicht helfen -->
...
Note the method called `transformToBlockData()`. This method transforms the block input into the savable block data. As this method is nearly identical for all blocks, we provide a `inputToData` method to create the intended outcome.
...
The `PageContenBlock` is created using the `createBlocksBlock` helper, which is one of multiple blocks helper. You may ignore the specifics of this root block for now.
...
<!-- wollten wir nicht eher createCompositeSettings präferieren? -->
...
For the **level** select, we're once again using a plain Final Form field in combination with createCompositeSetting.
...
export function PageContentBlock({ data }: PropsWithData<PageContentBlockData>) {
    return <BlocksBlock data={data} supportedBlocks={supportedBlocks} reportError={(error) => {
