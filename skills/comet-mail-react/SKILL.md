...
- **Nested levels** render as another table inside the enclosing item's text cell, so `list.indent` compounds per level. Every level draws the marker of its own list type. A level whose depth has no block above it to nest into renders at the top level.
