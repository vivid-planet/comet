...
configured by hand. `@comet/admin` exports `Future_DatePicker`, `Future_DateTimePicker`, and
`Future_TimePicker` (with `Future_DateRangePicker` and `DateTimeRangePicker` for ranges), plus
`Future_DatePickerField` and siblings for use as Final Form fields. Each picker
manages its own value format — `Future_DatePicker`, for example, reads and writes an ISO `YYYY-MM-DD`
...
The `Future_` prefix marks these as the upcoming default: they drop it in the next major, where they
replace the same-named pickers of `@comet/admin-date-time` (`DatePicker`, `DateField`,
`FinalFormDatePicker`, …), which are deprecated. Prefer the `Future_*` pickers from `@comet/admin`
in new code, and keep the legacy ones only where a screen already uses them.

```tsx
// Avoid — a plain text input used as a date field
<input type="text" value={value} onChange={(event) => onChange(event.target.value)} />;

// Prefer — a Future_DatePicker working with ISO date strings
<Future_DatePicker value={value} onChange={onChange} />;
