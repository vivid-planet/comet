---
name: dev-pm
description: Run, restart, stop, and inspect logs/status of long-running development processes via dev-pm (@comet/dev-process-manager). Use when starting/stopping/restarting services, tailing logs of those processes, or checking which services are running. Do not use for one-off scripts (`npm run X` is fine for those) — dev-pm is for processes that stay alive.
---

# dev-pm Skill

`dev-pm` (the CLI of `@comet/dev-process-manager`, published as `dev-process-manager` since its v4) supervises long-running dev processes (servers, watchers, codegen, storybook, docker, …). Available scripts and groups are defined in `dev-pm.config.ts` at the repo root — read it to discover what can be started.
