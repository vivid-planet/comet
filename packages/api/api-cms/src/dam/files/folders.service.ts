...
import { CometFolderNotEmptyException } from "./errors/folder-not-empty.exception";

@Injectable()
export class FoldersService {
    protected readonly logger = new Logger(FoldersService.name);

    constructor(@InjectRepository(Folder) private readonly foldersRepository: EntityRepository<Folder>) {}
    ...
    async delete(id: string): Promise<boolean> {
        const total = (
            await this.foldersRepository
                ...
                .select("folder.id")
                .join("folder.files", "file")
                .where({ id })
                .orWhere("? = ANY(folder.mpath)", [id])
                .getResult()
        ).length;
        if (total > 0) throw new CometFolderNotEmptyException();
