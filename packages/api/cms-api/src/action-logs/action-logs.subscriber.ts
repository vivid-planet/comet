...
import { GROUPED_ACTION_LOGS_METADATA_KEY, GroupedActionLogsMetadata } from "./grouped-action-logs.decorator";
...
export class ActionLogsSubscriber implements EventSubscriber {
    ...
    async onFlush(args: FlushEventArgs): Promise<void> {
        ...
        // because a grouped relation changed.
        ...
        for (const changeSet of args.uow.getChangeSets()) {
            ...
            // Editing an item of a grouped relation (without adding/removing it) produces a change set for the item but
            // not a collection update, so resolve the owning entity from the item's back-reference.
            const groupedRelationOwner = this.getGroupedRelationOwner(changeSet.entity);
            if (groupedRelationOwner && !entitiesToLog.has(groupedRelationOwner)) {
                entitiesToLog.set(groupedRelationOwner, undefined);
            }
        }

        // Adding/removing items of a grouped relation results in a collection update on the owning entity.
        for (const collection of args.uow.getCollectionUpdates()) {
            const owner = collection.owner;
            if (this.hasActionLogs(owner) && this.getGroupedRelations(owner).includes(collection.property.name) && !entitiesToLog.has(owner)) {
    ...
    async createLog(entity: AnyEntity, changeSet?: ChangeSet<AnyEntity>): Promise<ActionLog | null> {
        ...
        if (changeSet?.type !== ChangeSetType.DELETE) {
            const groupedRelations = this.getGroupedRelations(entity);
            const ignoreFields = entityMetadata.relations
                .map((entityProperty) => entityProperty.name)
                .filter((name) => !groupedRelations.includes(name));
    ...
    private getGroupedRelations(entity: AnyEntity): GroupedActionLogsMetadata {
        return Reflect.getMetadata(GROUPED_ACTION_LOGS_METADATA_KEY, entity.constructor.prototype) ?? [];
    }

    /**
     * Returns the owning entity if the given entity is an item of a grouped relation of an action-logged entity,
     * resolved via the item's to-one back-reference (e.g. `ProductColor.product`).
     */
    private getGroupedRelationOwner(entity: AnyEntity): AnyEntity | undefined {
        ...
        for (const relation of entityMetadata.relations) {
            ...
            const groupedRelations = this.getGroupedRelations(owner);
            const isGroupedRelation = ownerMetadata?.relations.some(
                (ownerRelation) => groupedRelations.includes(ownerRelation.name) && ownerRelation.mappedBy === relation.name,
            );
