...
export class UserPermissionResolver {
    ...
    async userPermissionsCreatePermission(
        ...
        return this.persistPermission(permission);
    }
    ...
    async persistPermission(permission: UserPermission): Promise<UserPermission> {
        await this.userService.checkContentScopes(permission.contentScopes ?? []);
