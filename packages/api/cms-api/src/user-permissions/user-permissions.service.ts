...
export class UserPermissionsService {
    constructor(
        @Inject(USER_PERMISSIONS_CONFIG_SERVICE) private readonly configService: UserPermissionConfigInterface,
    ...
    async getAvailableContentScopes(): Promise<ContentScope[]> {
        return this.configService.getAvailableContentScopes ? this.configService.getAvailableContentScopes() : [];
    }
    ...
    async getAvailablePermissions(): Promise<(keyof Permission)[]> {
        return [
            ...new Set<keyof Permission>([
                ...
                "userPermissions",
                "system",
                ...(this.configService.getAvailablePermissions ? await this.configService.getAvailablePermissions() : []),
            ]),
        ];
    }

    async getUser(id: string): Promise<User> {
        return this.configService.getUser(id);
    }

    async findUsers(args: FindUsersArgs): Promise<[User[], number]> {
        return this.configService.findUsers(args);
    }
    ...
    async getPermissions(userId: string): Promise<UserPermission[]> {
        ...
        if (this.configService.getPermissionsForUser) {
            const user = await this.getUser(userId);
            if (user) {
                let permissionsByRule = await this.configService.getPermissionsForUser(user);
    ...
    async getContentScopes(userId: string, skipManual = false): Promise<ContentScope[]> {
        ...
        if (this.configService.getContentScopesForUser) {
            const user = await this.getUser(userId);
            if (user) {
                const userContentScopes = await this.configService.getContentScopesForUser(user);
