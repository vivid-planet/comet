...
export class AccessLoggerInterceptor implements NestInterceptor {
    protected readonly logger = new Logger(AccessLoggerInterceptor.name);
    ...
    intercept(context: ExecutionContext, next: CallHandler) {
        ...
        if (requestType === "graphql") {
            ...
            if (user) additionalRequestData.push(`user: ${user.id} (${user.name})`);
            ...
            if (httpRequest.route.path.includes(DAM_IMAGE_PATH) || httpRequest.route.path.includes(DAM_FILE_PATH)) ignored = true;
