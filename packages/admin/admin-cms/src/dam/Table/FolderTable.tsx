...
import { FolderTableRow, isFile } from "./FolderTableRow";
    ...
    const tableColumns: ITableColumn<GQLDamFileTableFragment | GQLDamFolderTableFragment>[] = [
        {
            ...
            render: (row) => {
                return renderDamLabel ? (
                    ...
                    <Link underline="none" component={StackLink} pageName={isFile(row) ? "edit" : "folder"} payload={row.id}>
                        <DamLabel asset={row} showPath={isSearching} />
                    </Link>
                );
            },
        },
