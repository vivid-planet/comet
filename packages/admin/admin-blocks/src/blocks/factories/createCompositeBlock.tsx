...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type MergeBlockConfigs<T extends any> = { [BlockKey in AllBlockKeys<T>]: NonNullable<ExtractBlockConfig<T, BlockKey>> };
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export const createCompositeBlock = <Options extends CreateCompositeBlockOptions>(
    ...
    if (hasGroups(options)) {
        ...
        // TODO admin-blocks won't build without this (unnecessary) explicit cast
    ...
    // internally use the lower level composeapi-blocks
    const {
    ...
    const CompositeBlock: BlockInterface<BlockInputApi<typeof block>, BlockState<typeof block>, BlockOutputApi<typeof block>> = {
        ...createBlockSkeleton(),
        ...block, // merge in the blockMethods provided by the composeapi-blocks
