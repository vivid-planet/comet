...
export function generateGrid(
    ...
    const fieldNamesFromConfig: string[] = config.columns.map((field) => field.name);
                            ...
                            <CrudContextMenu
                                ...
                                onPaste={async ({ input }) => {
                                    await client.mutate<GQLCreate${gqlType}Mutation, GQLCreate${gqlType}MutationVariables>({
                                        ...
                                        variables: { ${hasScope ? `scope, ` : ""}input: filter(${instanceGqlTypePlural}Fragment, input) },
                                    });
                                }}
                                `
                                        : ""
