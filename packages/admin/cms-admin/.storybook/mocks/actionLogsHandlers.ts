...
export const actionLogsHandlers = [
    ...
    graphql.query("ActionLogsShowVersion", ({ variables }) =>
        HttpResponse.json({
            data: { actionLog: versionById[variables.id as string] ?? null },
        }),
    ),
    graphql.query("ActionLogsDialogGrid", () =>
        HttpResponse.json({
            data: {
    ...
    graphql.query("ActionLogsDialogShowVersion", ({ variables }) =>
        HttpResponse.json({
            data: { actionLog: versionById[variables.id as string] ?? null },
        }),
    ),
    graphql.query("ActionLogsDialogCompare", ({ variables }) =>
        HttpResponse.json({
            data: {
