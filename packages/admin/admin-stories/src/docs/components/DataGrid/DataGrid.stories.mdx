...
| initialSort       | Array<{ field: string; sort: GridSortDirection }> | InitialSort columns and sort directions applied to the DataGrid. Only applies the initalSort values if no other values are in the url. |
