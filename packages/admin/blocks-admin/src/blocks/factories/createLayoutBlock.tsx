...
export const createLayoutBlock = <Options extends CreateCompositeBlockOptions>(
    options: Options,
    override?: (
    ...
    const { name, displayName, category = BlockCategory.Other, layouts } = options;
    ...
    const CompositeBlock: BlockInterface<BlockInputApi<typeof block>, BlockState<typeof block>, BlockOutputApi<typeof block>> = {
        ...
        AdminComponent: ({ state, updateState }) => {
            ...
            // TODO: get visible blocks from current selected layout
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            const selectedLayout = state.layout;
            const visibleSelectedBlocks = layouts.find((layout) => layout.value === selectedLayout)?.visibleBlocks ?? [];
            const visibleBlocks = ["layout", ...visibleSelectedBlocks];
            const compareWithLayoutsConfig = (blockKey: string) => visibleBlocks.findIndex((blockName) => blockName === blockKey);
            const validKeys = new Set(visibleBlocks.map((blockName) => blockName));
                    ...
                    {[
                        <StackPage name="initial" key="initial">
                            {Object.entries(groups).map(([groupKey, group]) => {
                                ...
                                const children = Object.keys(blocks)
                                    .filter((blockKey) => validKeys.has(blockKey))
                                    .sort((a, b) => compareWithLayoutsConfig(a) - compareWithLayoutsConfig(b))
                                    .map((blockKey) => <Fragment key={blockKey}>{renderBlock(blockKey, group)}</Fragment>);
