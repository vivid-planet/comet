...
The Dextinity Admin `Tabs` are based on [Material UI's `Tabs`](https://mui.com/material-ui/react-tabs/) (`MuiTabs`).
...
## Differences between Dextinity Admin `Tabs` and `MuiTabs`

Like all Material UI components, `MuiTabs` is relatively low-level.
It's very flexible but has no built-in functionality.
Dextinity Admin enhances the `MuiTabs` by adding built-in functionality.
Therefore, using Dextinity Admin `Tabs` is considerably easier and faster.

The following example shows the definition of simple tabs with `MuiTabs` and Dextinity Admin `Tabs`.
...
return (
    <div>
        ...
        <div hidden={value !== 0}>{value === 0 && Content One}</div>
        <div hidden={value !== 1}>{value === 1 && Content Two}</div>
        <div hidden={value !== 2}>{value === 2 && Content Three}</div>
    </div>
);
```

With Dextinity Admin `Tabs`, the state management and rendering logic are built-in.
