...
type ChipSelectFieldPropsToExtendFrom<Value extends string | number | Record<string, string | number>> = FieldProps<Value, HTMLSelectElement>;

// Remove `children` from the interface. Omit cannot be used here because `FieldProps` contains an index signature.
type ChipSelectFieldPropsToExtendFromWithoutChildren<Value extends string | number | Record<string, string | number>> = {
    [K in keyof ChipSelectFieldPropsToExtendFrom<Value> as K extends "children" ? never : K]: ChipSelectFieldPropsToExtendFrom<Value>[K];
};

export interface ChipSelectFieldProps<Value extends string | number | Record<string, string | number>>
    extends ChipSelectFieldPropsToExtendFromWithoutChildren<Value> {
...
export function ChipSelectField<Value extends string | number | Record<string, string | number>>({
    componentsProps = {},
    children,
    fullWidth,
    ...restProps
