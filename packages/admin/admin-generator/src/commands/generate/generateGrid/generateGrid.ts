            ...
            ${gridColumnFields
                .map((column) => {
                    ...
                    if (column.type == "block" && hasPaging) {
                        // For block columns with paging, don't allow client-side filtering - only if API supports remote filtering
                        isColumnFilterable = filterFields.includes(column.name) || !!column.filterOperators;
                    }
                    ...
                    if (column.type == "block" && hasPaging) {
                        // For block columns with paging, don't allow client-side sorting - only if API supports remote sorting
                        isColumnSortable = sortFields.includes(column.name) || !!column.sortBy;
                    }
