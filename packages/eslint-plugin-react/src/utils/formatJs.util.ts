/* Format JS Utils functions copied from: https://github.com/formatjs/formatjs/blob/main/packages/eslint-plugin-formatjs/util.ts*/
...
function isTemplateLiteralWithoutVar(node: TSESTree.Node): node is TSESTree.TemplateLiteral {
    return node.type === "TemplateLiteral" && node.quasis.length === 1;
}

function staticallyEvaluateStringConcat(node: TSESTree.BinaryExpression): [result: string, isStaticallyEvaluatable: boolean] {
...
export function isIntlFormatMessageCall(node: TSESTree.Node): node is TSESTree.CallExpression {
    return (
...
function isSingleMessageDescriptorDeclaration(node: TSESTree.Node, functionNames: Set<string>) {
...
export function extractMessageDescriptor(node?: TSESTree.Expression): MessageDescriptorNodeInfo | undefined {
...
): [MessageDescriptorNodeInfo, TSESTree.ObjectExpression | undefined] | undefined {
...
export function extractMessages(
    node: TSESTree.Node,
    { additionalComponentNames, additionalFunctionNames, excludeMessageDeclCalls }: Settings = {},
    ...
    const allFormatFunctionNames = Array.isArray(additionalFunctionNames)
        ? new Set([...Array.from(FORMAT_FUNCTION_NAMES), ...additionalFunctionNames])
    ...
    if (node.type === "CallExpression") {
        ...
        if (
            (!excludeMessageDeclCalls && isSingleMessageDescriptorDeclaration(node, DECLARATION_FUNCTION_NAMES)) ||
...
export function patchMessage(
    ...
    if (messageNode.type === "Literal" && messageNode.value && typeof messageNode.value === "string") {
        return `"${patcher(messageNode.value as string, ast).replace('"', '\\"')}"`;
