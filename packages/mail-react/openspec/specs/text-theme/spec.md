...
- `color: string`
- `bottomSpacing: string` — must include a CSS unit (e.g. `"16px"`)
...
The default theme SHALL have `text.bottomSpacing` equal to `"16px"`.
...
- **WHEN** `createTheme()` is called with no arguments
- **THEN** the returned theme has `text.bottomSpacing === "16px"`
...
- **AND** unspecified text properties retain their defaults (`text.lineHeight === "20px"`, `text.bottomSpacing === "16px"`)
