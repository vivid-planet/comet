...
`TableQuery` has a built-in loading state and error handling.
All you have to do is passing the `api`, `error` and `loading` props returned by `useTableQuery` to `TableQuery`.

// TODO: More info concerning `useTableQuery`
