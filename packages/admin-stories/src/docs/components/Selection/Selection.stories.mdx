...
If you don't have all item data already loaded, you can provide a `query` and `dataAccessor` (instead of `rows`).
The item data is then automatically loaded from the API once an item is selected.
