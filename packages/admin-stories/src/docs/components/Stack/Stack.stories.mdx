...
A StackLink component is planned that will work similar to react-router Link.
...
`useStackSwitch` accesses the Context created by StackSwitch, for that reason you can not use it outside of StackSwitch, which also means not in the component where StackSwitch is created.

Comet Admin Stack provides a unique way to create a StackSwitch associated with it's api using `useStackSwitch` hook:
...
Toolbar components that integrates stack nicely into the Comet Toolbar System.
