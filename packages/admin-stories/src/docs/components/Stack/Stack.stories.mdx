...
It can be combined with the Material UI components [`Link`](https://material-ui.com/api/link/) and [`Button`](https://material-ui.com/api/button/) using their `component` prop.
More infos on this functionality can be found in the [Routing Libraries section of the Material UI Docs](https://material-ui.com/guides/composition/#routing-libraries).
...
| ...props   | Omit<ReactRouter.LinkProps, "to"> | Props of [ReactRouter.Link](https://reactrouter.com/web/api/Link) (except the `to` prop which is set by `StackLink`) |
