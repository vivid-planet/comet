...
export function WarningsGrid({ warningMessages: projectWarningMessages }: WarningsGridProps) {
    ...
    const columns: GridColDef<GQLWarningsListFragment>[] = [
        ...
        },
        {
            ...
            sortable: false,
            renderCell: ({ row }) => {
                const dependencyObject = entityDependencyMap[row.dependencyInfo.rootEntityName] as DependencyInterface | undefined;
                ...
                if (dependencyObject === undefined) {
                    if (process.env.NODE_ENV === "development") {
                        console.warn(
                            `Cannot load URL because no implementation of DependencyInterface for ${row.dependencyInfo.rootEntityName} was provided via the DependenciesConfig`,
                        );
                    }
