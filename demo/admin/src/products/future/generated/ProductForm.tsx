...
import { GQLProductFormDetailsFragmentVariables } from "./ProductForm.gql.generated";
...
export function ProductForm({ id }: FormProps) {
    ...
    const handleSubmit = async ({ dimensionsEnabled, ...formValues }: FormValues, form: FormApi<FormValues>, event: FinalFormSubmitEvent) => {
        ...
        const output = {
            ...formValues,
            category: formValues.category?.id, dimensions: dimensionsEnabled && formValues.dimensions ? { width: parseFloat(formValues.dimensions.width), height: parseFloat(formValues.dimensions.height), depth: parseFloat(formValues.dimensions.depth), } : null, manufacturer: formValues.manufacturer?.id, image: rootBlocks.image.state2Output(formValues.image), priceList: formValues.priceList ? formValues.priceList.id : null, datasheets: formValues.datasheets?.map(({ id }) => id),
        };
        if (mode === "edit") {
            if (!id)
                throw new Error();
    ...
    return (<FinalForm<FormValues> apiRef={formApiRef} onSubmit={handleSubmit} mode={mode} initialValues={initialValues} initialValuesEqual={isEqual} //required to compare block data correctly
     subscription={{ values: true }}>
        ...
        <TextAreaField required variant="horizontal" fullWidth name="description" label={<FormattedMessage id="product.description" defaultMessage="Description"/>}/>
