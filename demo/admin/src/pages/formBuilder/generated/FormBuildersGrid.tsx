...
const formBuildersQuery = gql`
    query FormBuildersGrid($offset: Int!, $limit: Int!, $sort: [FormBuilderSort!], $search: String, $filter: FormBuilderFilter) {
        formBuilders(offset: $offset, limit: $limit, sort: $sort, search: $search, filter: $filter) {
            nodes {
                ...FormBuildersGrid
            }
            totalCount
...
export function FormBuildersGrid({ toolbarAction }: Props): React.ReactElement {
    ...
    const columns: GridColDef<GQLFormBuildersGridFragment>[] = [
        ...
        },
        {
            ...
            type: "date",
            valueGetter: ({ row }) => row.updatedAt && new Date(row.updatedAt),
            valueFormatter: ({ value }) => (value ? intl.formatDate(value) : ""),
            flex: 1,
            minWidth: 150,
        },
