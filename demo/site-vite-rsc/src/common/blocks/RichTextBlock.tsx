"use client";
import { hasRichTextBlockContent, PreviewSkeleton, type PropsWithData, withPreview } from "@comet/site-react";
import { type LinkBlockData, type RichTextBlockData } from "@src/blocks.generated";
import { PageLayout } from "@src/layout/PageLayout";
import clsx from "clsx";
import { default as defaultRedraft, type Renderers, type TextBlockRenderFn } from "redraft";
...
const redraft = (defaultRedraft as any).default || defaultRedraft; //TODO client vs server side issue?!
...
const defaultRichTextRenderers: Renderers = {
    ...
    blocks: {
        ...
        "ordered-list-item": (children, { depth, keys }) => (
            <ol key={keys.join("|")} className={styles[`orderedListLevel${depth % 3}`]}>
                {children.map((child, index) => (
                    <Typography as="li" key={keys[index]} className={styles.text}>
                        {child}
                    </Typography>
