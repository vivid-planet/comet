...
export class ProductResolver {
    ...
    private readonly productStatisticsRepository: EntityRepository<ProductStatistics>, 
    @InjectRepository(ProductColor)
    private readonly productColorRepository: EntityRepository<ProductColor>, 
    @InjectRepository(ProductToTag)
    private readonly productToTagRepository: EntityRepository<ProductToTag>, 
    @InjectRepository(ProductTag)
    private readonly productTagRepository: EntityRepository<ProductTag>, private readonly blocksTransformer: BlocksTransformerService) { }
    @Query(() => Product)
    ...
    input: ProductUpdateInput): Promise<Product> {
        ...
        if (categoryInput !== undefined) {
            product.category =
                categoryInput ?
                    Reference.create(await this.productCategoryRepository.findOneOrFail(categoryInput))
                    : undefined;
        }
        ...
        if (priceListInput !== undefined) {
            product.priceList =
                priceListInput ?
                    Reference.create(await this.fileUploadRepository.findOneOrFail(priceListInput))
                    : undefined;
        }
        if (imageInput) {
