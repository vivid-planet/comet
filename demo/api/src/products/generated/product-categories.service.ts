...
export class ProductCategoriesService {
    ...
    private readonly repository: EntityRepository<ProductCategory>) { }
    async incrementPositions(lowestPosition: number, highestPosition?: number) {
        ...
        await this.repository.nativeUpdate({ position: { $gte: lowestPosition, ...(highestPosition ? { $lt: highestPosition } : {}) } }, { position: raw("position + 1") });
    }
    async decrementPositions(lowestPosition: number, highestPosition?: number) {
        ...
        await this.repository.nativeUpdate({ position: { $gt: lowestPosition, ...(highestPosition ? { $lte: highestPosition } : {}) } }, { position: raw("position - 1") });
    }
