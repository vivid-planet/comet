...
type AiGeneration = "AiGenerated" | "AiModified";
...
const labels: Record<AiGeneration, { id: string; defaultMessage: string }> = {
    ...
    AiModified: { id: "aiGeneratedBadge.aiModified", defaultMessage: "AI modified" },
};
 ...
 * applicable from 2 August 2026. The label distinguishes the two cases the official EU icon set
 * covers (AI-generated, AI-modified). The freely usable EU icons can replace the inline mark below:
...
const AiIcon = ({ variant }: { variant: AiGeneration }) => {
    const filled = variant === "AiGenerated";
    return (
        <svg className={styles.icon} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">
            ...
            {variant === "AiModified" && <circle cx="19" cy="5" r="3.4" fill="currentColor" stroke="#000" strokeWidth="0.8" />}
        </svg>
    );
};
